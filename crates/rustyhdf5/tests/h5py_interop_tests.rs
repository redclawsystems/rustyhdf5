//! Bidirectional interop tests between rustyhdf5 and h5py (Python).
//!
//! Tests are skipped if python3 or h5py are not available.

use std::process::Command;

use rustyhdf5::{AttrValue, CompoundTypeBuilder, DType, File, FileBuilder};
...
// ===========================================================================
// Part A: rustyhdf5 writes -> h5py reads
// ===========================================================================
...
#[test]
fn rustyhdf5_writes_f64_h5py_reads() {
    skip_if_no_python!();
...
#[test]
fn rustyhdf5_writes_strings_h5py_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("strings_test.h5");
    let path_str = path.display().to_string();

    // rustyhdf5 doesn't have a direct with_string_data, so we write f64 and
...
#[test]
fn rustyhdf5_writes_chunked_compressed_h5py_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
#[test]
fn rustyhdf5_writes_groups_attrs_h5py_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
#[test]
fn rustyhdf5_writes_compound_h5py_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
// ===========================================================================
// Part B: h5py writes -> rustyhdf5 reads
// ===========================================================================

// ---------------------------------------------------------------------------
// B6. h5py creates f64 dataset -> rustyhdf5 reads -> verify
// ---------------------------------------------------------------------------

#[test]
fn h5py_writes_f64_rustyhdf5_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
// ---------------------------------------------------------------------------
// B7. h5py creates string dataset -> rustyhdf5 reads -> verify
// ---------------------------------------------------------------------------

#[test]
fn h5py_writes_strings_rustyhdf5_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("h5py_strings.h5");
    let path_str = path.display().to_string();

    // Use fixed-length strings (S10) since rustyhdf5 doesn't support vlen strings yet
    let script = format!(
        r#"
import h5py, numpy as np
with h5py.File("{path_str}", "w") as f:
...
// ---------------------------------------------------------------------------
// B8. h5py creates chunked+compressed+shuffled -> rustyhdf5 reads -> verify
// ---------------------------------------------------------------------------

#[test]
fn h5py_writes_chunked_compressed_rustyhdf5_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
// ---------------------------------------------------------------------------
// B9. h5py creates nested groups with attrs -> rustyhdf5 reads -> verify
// ---------------------------------------------------------------------------

#[test]
fn h5py_writes_nested_groups_rustyhdf5_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
...
// ---------------------------------------------------------------------------
// B10. h5py creates multiple dtypes -> rustyhdf5 reads all -> verify
// ---------------------------------------------------------------------------

#[test]
fn h5py_writes_multiple_dtypes_rustyhdf5_reads() {
    skip_if_no_python!();
    let dir = tempfile::tempdir().unwrap();
