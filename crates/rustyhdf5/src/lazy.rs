...
//! [`LazyFile`] works with any [`rustyhdf5_io::HDF5Read`] backend (including
...
use rustyhdf5_format::group_v1::{self, GroupEntry};
use rustyhdf5_format::group_v2;
...
impl LazyFile<rustyhdf5_io::MemoryReader> {
    /// Open a lazy file from an in-memory byte vector.
    pub fn from_bytes(data: Vec<u8>) -> Result<Self, Error> {
        let reader = rustyhdf5_io::MemoryReader::new(data);
        Self::open(reader)
    }
}

#[cfg(feature = "mmap")]
impl LazyFile<rustyhdf5_io::MmapReader> {
    ...
    pub fn open_mmap<P: AsRef<std::path::Path>>(path: P) -> Result<Self, Error> {
        let reader = rustyhdf5_io::MmapReader::open(path).map_err(Error::Io)?;
        Self::open(reader)
    }
}
...
) -> Result<&rustyhdf5_format::object_header::HeaderMessage, Error> {
    header
        .messages
        .iter()
