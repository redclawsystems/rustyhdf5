...
fn compute_row_copies(
    ...
    let outer_dims = &chunk_dims[..inner_dim];
    let outer_count: usize = outer_dims.iter().product();

    // Compute outer-only strides (row-major within the outer dimensions only).
    // These are different from chunk_strides which include the innermost dim.
    let mut outer_strides = vec![1usize; inner_dim];
    for i in (0..inner_dim.saturating_sub(1)).rev() {
        outer_strides[i] = outer_strides[i + 1] * outer_dims[i + 1];
    }
    ...
    for outer_flat in 0..outer_count {
        ...
        for d in 0..inner_dim {
            let coord_in_chunk = remaining / outer_strides[d];
