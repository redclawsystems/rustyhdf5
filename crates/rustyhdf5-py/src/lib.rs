...
impl From<OwnedAttrValue> for rustyhdf5_rs::AttrValue {
    fn from(v: OwnedAttrValue) -> Self {
        match v {
            OwnedAttrValue::F64(x) => rustyhdf5_rs::AttrValue::F64(x),
            OwnedAttrValue::I64(x) => rustyhdf5_rs::AttrValue::I64(x),
            OwnedAttrValue::Str(s) => rustyhdf5_rs::AttrValue::String(s),
            OwnedAttrValue::F64Array(a) => rustyhdf5_rs::AttrValue::F64Array(a),
            OwnedAttrValue::I64Array(a) => rustyhdf5_rs::AttrValue::I64Array(a),
        }
    }
}
...
pub(crate) fn attr_value_to_py(py: Python<'_>, val: &rustyhdf5_rs::AttrValue) -> PyObject {
    match val {
        rustyhdf5_rs::AttrValue::F64(v) => v.into_pyobject(py).unwrap().into_any().unbind(),
        rustyhdf5_rs::AttrValue::I64(v) => v.into_pyobject(py).unwrap().into_any().unbind(),
        rustyhdf5_rs::AttrValue::U64(v) => v.into_pyobject(py).unwrap().into_any().unbind(),
        rustyhdf5_rs::AttrValue::String(s) => s.into_pyobject(py).unwrap().into_any().unbind(),
        rustyhdf5_rs::AttrValue::F64Array(a) => {
            let list = pyo3::types::PyList::new(py, a).unwrap();
            list.into_any().unbind()
        }
        rustyhdf5_rs::AttrValue::I64Array(a) => {
            let list = pyo3::types::PyList::new(py, a).unwrap();
            list.into_any().unbind()
        }
        rustyhdf5_rs::AttrValue::StringArray(a) => {
            let list = pyo3::types::PyList::new(py, a).unwrap();
            list.into_any().unbind()
        }
    }
...
pub(crate) fn apply_dataset_spec(
    db: &mut rustyhdf5_format::type_builders::DatasetBuilder,
    spec: &DatasetSpec,
) {
    match &spec.data {
        DatasetData::F64(v) => {
...
/// The rustyhdf5 Python module.
#[pymodule]
fn rustyhdf5(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<PyFile>()?;
...
mod tests {
    ...
    fn owned_attr_value_roundtrip() {
        ...
        let attr: rustyhdf5_rs::AttrValue = val.into();
        assert!(matches!(attr, rustyhdf5_rs::AttrValue::F64(v) if (v - 3.14).abs() < 1e-10));
    }
    ...
    fn owned_attr_value_i64() {
        ...
        let attr: rustyhdf5_rs::AttrValue = val.into();
        assert!(matches!(attr, rustyhdf5_rs::AttrValue::I64(42)));
    }
    ...
    fn owned_attr_value_str() {
        ...
        let attr: rustyhdf5_rs::AttrValue = val.into();
        assert!(matches!(attr, rustyhdf5_rs::AttrValue::String(ref s) if s == "hello"));
    }
    ...
    fn owned_attr_value_f64_array() {
        ...
        let attr: rustyhdf5_rs::AttrValue = val.into();
        assert!(matches!(attr, rustyhdf5_rs::AttrValue::F64Array(ref v) if v == &[1.0, 2.0]));
    }
    ...
    fn owned_attr_value_i64_array() {
        ...
        let attr: rustyhdf5_rs::AttrValue = val.into();
        assert!(matches!(attr, rustyhdf5_rs::AttrValue::I64Array(ref v) if v == &[1, 2, 3]));
    }
    ...
    fn dataset_spec_apply() {
        ...
        let file = rustyhdf5_rs::File::from_bytes(bytes).unwrap();
