...
/// as a byte slice, which is the interface that `rustyhdf5-format` expects.
pub trait HDF5Read {
...
///
/// This mirrors what `rustyhdf5-format` currently does: the entire file
...
mod tests {
    ...
    fn file_reader_roundtrip() {
        ...
        let path = dir.join("rustyhdf5_io_test_file_reader.bin");
    ...
    fn file_reader_from_file() {
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_from_file.bin");
    ...
    fn file_reader_nonexistent() {
        let result = FileReader::open("/tmp/rustyhdf5_io_does_not_exist_12345.bin");
    ...
    fn file_writer_create_and_write() {
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_writer.bin");
    ...
    fn file_writer_overwrite() {
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_writer_overwrite.bin");
    ...
    fn file_writer_path() {
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_path.bin");
    ...
    fn file_writer_flush_to_disk() {
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_flush.bin");
    ...
    fn hdf5_file_via_memory_reader() {
        // Integration test: use MemoryReader with an HDF5 file created by rustyhdf5-format
    ...
    fn hdf5_file_via_file_reader_writer() {
        ...
        let dir = std::env::temp_dir();
        let path = dir.join("rustyhdf5_io_test_hdf5_roundtrip.h5");
